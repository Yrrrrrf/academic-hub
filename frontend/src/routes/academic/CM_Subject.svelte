<script lang="ts">
	...
	tabindex="0"
	>
	<div class="w-full h-28 {color} rounded-lg shadow-md overflow-hidden relative border-l-4 {typeColors[subject.category]}"> <!-- Added border for type indication -->
	<div class="p-2 h-full flex flex-col justify-between">
		...
		<div>
			...
			<span class="ml-2 font-semibold capitalize">{subject.category}</span> <!-- Added type text -->
			</p>
			{#if grade !== undefined}
			<p class="text-xs mb-1">Grade: <span class="font-semibold">{grade}</span></p>
			{/if}
			<p class="text-xs mb-1">{truncate(subject.description, 100)}</p> <!-- Added truncated description -->
		</div>
